((canvas) => {
    ...
    class Cell {
      ...
      setColors () {
        ...
        this.color = hsla(Math.floor(map(this.i, 0, this.board.cells.length, fg[0] + 50, fg[0])), ...fg.slice(1))
        this.bg = hsla(...bg)
      }
    }
    
    const board = new Board(canvas, 5, {bg: [0, 0, 100, .015], alive: [174, 61, 55, 0.8]})
    board.init()
    board.start()
